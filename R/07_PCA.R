...
##
##  Files to keep next to this script: BiodivCountries.csv; CitiesEurope.csv; NeotomaMorphoEnvir.csv; speciesCrawley3.csv
##
...
#   crawley = read.csv("speciesCrawley3.csv", stringsAsFactors = TRUE),
#   europe  = read.csv("CitiesEurope.csv", stringsAsFactors = TRUE),
#   iris    = { utils::data("iris");   iris },
#   neotoma = read.csv("NeotomaMorphoEnvir.csv", stringsAsFactors = TRUE),
#   stop("unknown data set: ", name))
# data_dir <- function() "."                       # look for data files here
## ---------------------------------------------------------------------------
if (!exists("get_data")) source(file.path("R", "00_utils.R"))
need("MASS", "vegan", "cluster")
...
} else skip_note("scatterplot matrix of the bioclim block", "car")
...
with_fig("07_screeplot_princomp", {
  par(mfrow = c(1, 2))
...
with_fig("07_evplot", evplot(pc_bio$sdev^2, "(19 bioclim variables)"),
         width = 8, height = 7)
...
with_fig("07_block_scree_scores", {
...
with_fig("07_procrustes_specimens", {
...
with_fig("07_procrustes_species", {
  par(mfrow = c(1, 2))
...
with_fig("07_procrustes_residuals", {
  par(mfrow = c(1, 2))
...
if (interactive() && has_pkg("rgl")) {
  ...
  rgl::aspect3d("iso")
} else skip_note("3-D confidence ellipsoid with its semi-axes", "rgl")
...
with_fig("07_screeplot_biodiv", screeplot(pcaB, main = "Biodiversity PCA"))
...
with_fig("07_pcoa_cities_bstick", {
...
if (has_pkg("ape")) {
  ...
  with_fig("07_pcoa_biplot", {
    ape::biplot.pcoa(pcoaa, biodata[, 2:5], dir.axis2 = -1)
  }, width = 8, height = 7)
} else skip_note("ape::pcoa and its biplot", "ape")
...
vcf_path <- file.path(data_dir(), "gen.10000.vcf")
if (file.exists(vcf_path) && has_pkg("vcfR", "adegenet", "ade4", "cluster")) {
  ...
  skip_note("the genomic PCA/PCoA comparison (needs gen.10000.vcf)",
            c("vcfR", "adegenet"))
}
